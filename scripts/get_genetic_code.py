...
def taxon2geneticcode(taxon_id, query_email):
    """Resolve the nuclear genetic code for a taxon id.

    Importable counterpart to get_busco_db.taxon2lineage: sets the Entrez email
    and returns the integer translation table (or None if it can't be resolved).
    """
    Entrez.email = query_email
    return get_genetic_code(taxon_id)
